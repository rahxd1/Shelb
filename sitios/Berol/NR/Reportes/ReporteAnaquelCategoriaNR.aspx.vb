...
Partial Public Class ReporteAnaquelCategoriaNR
    ...
    Sub CargarReporte()
        ...
        If cmbPeriodo.SelectedValue <> "" Then
            ...
            CargaGrilla(ConexionBerol.localSqlServer, _
                        "select TPROD.nombre_tipo,  " & _
                        "convert(nvarchar(10),round(100*HDET1.cantidad/(HDET1.cantidad+HDET2.cantidad),2))+ '%' as NR, " & _
                        "convert(nvarchar(10),round(100*HDET2.cantidad/(HDET1.cantidad+HDET2.cantidad),2))+ '%' as Competencia " & _
                        "FROM Tipo_Productos as TPROD " & _
                        "FULL JOIN (select H.tipo_producto, SUM(H.cantidad)cantidad " & _
                        "FROM View_Historial_NR as H  " & _
                        " " + Berol.CuentaClave + " " & _
                        "WHERE H.propia=1 AND H.id_periodo=" & cmbPeriodo.SelectedValue & " " & _
                        " " + RegionSQL + EstadoSQL + " " & _
                        " " + SupervisorSQL + PromotorSQL + " " & _
                        " " + CadenaSQL + FormatoSQL + TiendaSQL + " " & _
                        " " + TiendaSQL + " " & _
                        "GROUP BY H.tipo_producto) as HDET1 " & _
                        "ON TPROD.tipo_producto=HDET1.tipo_producto " & _
                        "FULL JOIN (select H.tipo_producto, SUM(H.cantidad)cantidad " & _
                        "FROM View_Historial_NR as H  " & _
                        " " + Berol.CuentaClave + " " & _
                        "WHERE H.propia=2 AND H.id_periodo=" & cmbPeriodo.SelectedValue & " " & _
                        " " + RegionSQL + EstadoSQL + " " & _
                        " " + SupervisorSQL + PromotorSQL + " " & _
                        " " + CadenaSQL + FormatoSQL + TiendaSQL + " " & _
                        " " + TiendaSQL + " " & _
                        "GROUP BY H.tipo_producto) as HDET2 " & _
                        "ON TPROD.tipo_producto=HDET2.tipo_producto " & _
                        "ORDER BY TPROD.nombre_tipo ", Me.gridReporte)
        Else
    ...
    Protected Sub lnkExportar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkExportar.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=Reporte participación en anaquel por categoria " + cmbPeriodo.SelectedItem.ToString() + ".xls")
