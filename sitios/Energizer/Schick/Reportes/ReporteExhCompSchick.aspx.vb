...
Partial Public Class ReporteExhCompSchick
    ...
    Sub CargarReporte()
        Using cnn As New SqlConnection(ConexionEnergizer.localSqlServer)
            ...
            Dim sql As String = "SELECT DISTINCT REG.nombre_region as Region, CAD.nombre_cadena as Cadena, TI.nombre as Tienda, " & _
                        "HDET1.cantidad as Schick,  " & _
                        "HDET3.cantidad_exhibidor as GILLETE, HDET4.cantidad_exhibidor as BIC, HDET5.cantidad_exhibidor as OTRAS " & _
                        "FROM Schick_Historial as H  " & _
                        "FULL JOIN (select DISTINCT H.id_tienda, H.folio_historial, sum(HDET.cantidad) as cantidad FROM Schick_Historial as H " & _
                        "INNER JOIN Schick_Exhibidores_Historial_Det as HDET ON H.folio_historial= HDET.folio_historial " & _
                        "WHERE H.id_periodo = " & cmbPeriodo.SelectedValue & " " & _
                        "GROUP BY H.id_tienda, H.folio_historial) as HDET1 " & _
                        "ON H.id_tienda = HDET1.id_tienda " & _
                        "FULL JOIN (select H.id_tienda, H.folio_historial, HDET.cantidad_exhibidor FROM Schick_Historial as H " & _
                        "INNER JOIN Schick_Competencia_Historial_Det as HDET ON H.folio_historial= HDET.folio_historial " & _
                        "WHERE H.id_periodo = " & cmbPeriodo.SelectedValue & " AND HDET.id_marca =2) as HDET3 " & _
                        "ON H.id_tienda = HDET3.id_tienda " & _
                        "FULL JOIN (select H.id_tienda, H.folio_historial, HDET.cantidad_exhibidor FROM Schick_Historial as H " & _
                        "INNER JOIN Schick_Competencia_Historial_Det as HDET ON H.folio_historial= HDET.folio_historial " & _
                        "WHERE H.id_periodo = " & cmbPeriodo.SelectedValue & " AND HDET.id_marca =3) as HDET4 " & _
                        "ON H.id_tienda = HDET4.id_tienda " & _
                        "FULL JOIN (select H.id_tienda, H.folio_historial, HDET.cantidad_exhibidor FROM Schick_Historial as H " & _
                        "INNER JOIN Schick_Competencia_Historial_Det as HDET ON H.folio_historial= HDET.folio_historial " & _
                        "WHERE H.id_periodo = " & cmbPeriodo.SelectedValue & " AND HDET.id_marca =4) as HDET5 " & _
    ...
    Protected Sub lnkExportar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkExportar.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=Reporte Exhibidores Competencia " + cmbPeriodo.SelectedItem.ToString + ".xls")
