...
Partial Public Class ReporteGaleriaSchick
    ...
    Sub SQLCombo()
        ...
        PeriodoSQL = "SELECT DISTINCT PER.id_periodo, PER.nombre_periodo " & _
                    "FROM Schick_Galerias_Historial as H " & _
                    "INNER JOIN Schick_Periodos as PER ON PER.id_periodo = H.id_periodo" & _
                    " ORDER BY id_periodo DESC"

        RegionSQL = "SELECT DISTINCT REG.nombre_region, REG.id_region " & _
                    "FROM Regiones as REG " & _
                    "INNER JOIN Schick_Tiendas AS TI ON REG.id_region = TI.id_region " & _
                    "INNER JOIN Schick_Galerias_Historial as H ON H.id_tienda = TI.id_tienda " & _
        ...
        CadenaSQL = "SELECT DISTINCT CAD.id_cadena, CAD.nombre_cadena " & _
                    ...
                    "INNER JOIN Schick_Tiendas AS TI ON TI.id_cadena = CAD.id_cadena " & _
                    "INNER JOIN Regiones as REG ON REG.id_region = TI.id_region " & _
                    "INNER JOIN Schick_Galerias_Historial AS H ON TI.id_tienda = H.id_tienda " & _
        ...
        TiendaSQL = "SELECT DISTINCT TI.id_tienda, TI.nombre " & _
                   "FROM Schick_Galerias_Historial AS H " & _
                   "INNER JOIN Schick_Tiendas AS TI ON H.id_tienda = TI.id_tienda " & _
    ...
    Sub CargarFotos()
        ...
        Using cnn As New SqlConnection(ConexionEnergizer.localSqlServer)
            ...
            GridSQL = "SELECT DISTINCT PER.id_periodo,PER.nombre_periodo, TI.id_region,REG.nombre_region, TI.id_cadena,CAD.nombre_cadena, TI.id_tienda,TI.nombre, GAL.ruta , GAL.foto, GAL.descripcion " & _
                "FROM Schick_Galerias_Historial as GAL " & _
                "INNER JOIN Schick_Tiendas as TI ON GAL.id_tienda = TI.id_tienda " & _
                "INNER JOIN Cadenas_Tiendas as CAD ON CAD.id_cadena =  TI.id_cadena " & _
                "INNER JOIN Regiones as REG ON REG.id_region = Ti.id_region " & _
                "INNER JOIN Schick_Periodos as PER ON PER.id_periodo= GAL.id_periodo " & _
                "WHERE TI.estatus = 1 " & _
            ...
            Dim cmd As New SqlCommand(GridSQL, cnn)
            Dim adapter As New SqlDataAdapter
            adapter.SelectCommand = cmd
            cnn.Open()
            Dim dataset As New DataSet
            adapter.Fill(dataset, "id_tienda")
            cnn.Close()
            gridImagenes.DataSource = dataset
