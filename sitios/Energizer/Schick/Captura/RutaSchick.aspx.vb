...
Partial Public Class RutaSchick
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            If tipo_usuario <> 1 Then
                ...
                ''//Cargar promotor
                Combo.LlenaDrop(ConexionEnergizer.localSqlServer, "SELECT DISTINCT id_usuario FROM Usuarios WHERE id_proyecto=32 ORDER BY id_usuario", "id_usuario", "id_usuario", cmbPromotor)
                ''//Cargar Periodo Promotor Supervisor
                Combo.LlenaDrop(ConexionEnergizer.localSqlServer, "SELECT * FROM Schick_Periodos ORDER BY id_periodo DESC", "nombre_periodo", "id_periodo", cmbPeriodo)
                Region()
                cmbPromotor.Items.Insert(0, New ListItem("", ""))
            Else
                ''//Cargar periodo promotor
                Combo.LlenaDrop(ConexionEnergizer.localSqlServer, "SELECT TOP 4 * FROM Schick_Periodos order by id_periodo DESC", "nombre_periodo", "id_periodo", cmbPeriodo)
    ...
    Sub CargarRuta()
        ...
        Using cnn As New SqlConnection(ConexionEnergizer.localSqlServer)
            ...
            SQLCargaRuta = "execute pcdt_Schick_Cargar_Ruta '" & cmbPeriodo.SelectedValue & "','" & Usuario & "'"
            Dim cmd As New SqlCommand(SQLCargaRuta, cnn)
            Dim adapter As New SqlDataAdapter
            adapter.SelectCommand = cmd
            Dim dataset As New DataSet
            adapter.Fill(dataset, "Schick_Tiendas")
            gridRutas.DataSource = dataset
            gridRutas.DataBind()
            dataset.Dispose()
            cmd.Dispose()
            adapter.Dispose()
    ...
    Protected Sub cmbRegion_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbRegion.SelectedIndexChanged
        If cmbRegion.SelectedValue <> 0 Then
            Combo.LlenaDrop(ConexionEnergizer.localSqlServer, "SELECT DISTINCT RE.id_usuario FROM Schick_Rutas_Eventos AS RE " & _
    ...
    Private Sub PeriodoActivo()
        ...
        cnn.Open()
        Dim SQL As New SqlCommand("SELECT * FROM Schick_Periodos where id_periodo =" & cmbPeriodo.Text & "", cnn)
