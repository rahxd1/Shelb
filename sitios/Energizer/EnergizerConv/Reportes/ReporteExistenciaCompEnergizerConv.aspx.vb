...
Partial Public Class ReporteExistenciaCompEnergizerConv
    ...
    Sub CargarReporte()
        If Not cmbPeriodo.SelectedValue = "" Then
            PeriodoSQL = "AND Datos.id_periodo=" & cmbPeriodo.SelectedValue & " " : Else
            PeriodoSQL = "" : End If

        If cmbRegion.SelectedValue <> 0 Then
            RegionSQL = "AND Datos.id_region=" & cmbRegion.SelectedValue & " " : Else
            RegionSQL = "" : End If

        If Not cmbPromotor.SelectedValue = "" Then
            PromotorSQL = "AND Datos.id_usuario='" & cmbPromotor.SelectedValue & "' " : Else
            PromotorSQL = "" : End If

        If Not cmbTienda.SelectedValue = "" Then
            TiendaSQL = "AND Datos.id_tienda=" & cmbTienda.SelectedValue & " " : Else
            TiendaSQL = "" : End If

        CargaGrilla(ConexionEnergizer.localSqlServer, _
                    "select * from " & _
                    "(SELECT DISTINCT PER.nombre_periodo,H.id_periodo, TI.id_region, REG.nombre_region, H.id_usuario, H.folio_historial, CAD.nombre_cadena, TI.id_tienda, Ti.nombre " & _
                    ...
                    "INNER JOIN Regiones as REG ON REG.id_region= TI.id_region " & _
                    "INNER JOIN Energizer_Productos as PROD ON PROD.id_producto= HDET.id_producto " & _
                    "INNER JOIN Energizer_Conv_Productos as PRODENE ON PROD.id_producto= PRODENE.id_producto) as Datos, " & _
                    "(SELECT sum(HDET.existencia) as ExistenciasTotales, HDET.folio_historial " & _
                    "From Energizer_Conv_Productos_Historial_Det as HDET " & _
                    "INNER JOIN Energizer_Productos as PROD ON PROD.id_producto= HDET.id_producto " & _
                    "INNER JOIN Energizer_Conv_Productos as PRODENE ON PROD.id_producto= PRODENE.id_producto " & _
                    "WHERE(PROD.tipo_producto = 1) " & _
                    "group by HDET.folio_historial) AS Energizer, " & _
                    "(SELECT sum(HDET.existencia) as Existenciascompetencia, HDET.folio_historial " & _
                    "From Energizer_Conv_Productos_Historial_Det as HDET " & _
                    "INNER JOIN Energizer_Productos as PROD ON PROD.id_producto= HDET.id_producto " & _
                    "INNER JOIN Energizer_Conv_Productos as PRODENE ON PROD.id_producto= PRODENE.id_producto " & _
                    "WHERE(PROD.tipo_producto = 2)" & _
                    "group by HDET.folio_historial) AS Competencia " & _
                    "WHERE(Energizer.folio_historial = Datos.folio_historial)" & _
                    "AND Competencia.folio_historial = Datos.folio_historial " & _
                    " " + PeriodoSQL + " " & _
                    " " + RegionSQL + " " & _
                    " " + PromotorSQL + " " & _
                    " " + TiendaSQL + " " & _
                    "ORDER BY nombre_periodo,nombre_region,nombre ", Me.gridReporte)
    End Sub
