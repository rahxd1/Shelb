...
Partial Public Class RutaEnergizerDP
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            If tipo_usuario <> 1 Then
                ...
                ''//Cargar promotor
                Combo.LlenaDrop(ConexionEnergizer.localSqlServer, "SELECT DISTINCT id_usuario FROM Usuarios WHERE id_proyecto=11 ORDER BY id_usuario", "id_usuario", "id_usuario", cmbPromotor)
                ''//Cargar Periodo Promotor Supervisor
                Combo.LlenaDrop(ConexionEnergizer.localSqlServer, "SELECT * FROM Energizer_DP_Periodos ORDER BY id_periodo DESC", "nombre_periodo", "id_periodo", cmbPeriodo)
                Region()
            Else
                ''//Cargar periodo promotor
                Combo.LlenaDrop(ConexionEnergizer.localSqlServer, "SELECT TOP 4 * FROM Energizer_DP_Periodos order by id_periodo DESC", "nombre_periodo", "id_periodo", cmbPeriodo)
            End If
    ...
    Sub CargarRuta()
        ...
        CargaGrilla(ConexionEnergizer.localSqlServer, _
                    "execute pcdt_Energizer_DP_cargar_ruta " & cmbPeriodo.SelectedValue & ",'" & Usuario & "'", _
                    gridRutas)
    End Sub
    ...
    Protected Sub cmbRegion_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbRegion.SelectedIndexChanged
        If cmbRegion.SelectedValue <> 0 Then
            Combo.LlenaDrop(ConexionEnergizer.localSqlServer, "SELECT DISTINCT RE.id_usuario FROM Energizer_DP_Rutas_Eventos AS RE " & _
                        "INNER JOIN Energizer_DP_Tiendas AS TI ON TI.id_tienda = RE.id_tienda " & _
    ...
    Sub Region()
        Combo.LlenaDrop(ConexionEnergizer.localSqlServer, "SELECT DISTINCT REG.nombre_region, REG.id_region " & _
                    "FROM Energizer_DP_Rutas_Eventos AS RE " & _
                    "INNER JOIN Energizer_DP_Tiendas AS TI ON TI.id_tienda = RE.id_tienda " & _
                    "INNER JOIN Regiones as REG ON REG.id_region = TI.id_region " & _
                    "INNER JOIN Cadenas_Tiendas as CAD ON TI.id_cadena = CAD.id_cadena " & _
    ...
    Private Sub PeriodoActivo()
        ...
        cnn.Open()
        Dim SQL As New SqlCommand("SELECT * FROM Energizer_DP_Periodos where id_periodo =" & cmbPeriodo.Text & "", cnn)
