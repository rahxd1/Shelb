...
Partial Public Class ReporteEnergizerDPPromComp
    ...
    Sub CargarReporte()
        ...
        CargaGrilla(ConexionEnergizer.localSqlServer, _
                    "SELECT PER.nombre_periodo, PER.nombre_periodo, REG.nombre_region, H.id_usuario, CAD.nombre_cadena, Ti.nombre, COMP.nombre_competencia,HCOMP.tipo_promocionales " & _
                    "FROM Energizer_DP_Productos_Historial as H " & _
                    "INNER JOIN Energizer_DP_Productos_Historial_Comp as HCOMP ON H.folio_historial = HCOMP.folio_historial " & _
                    "INNER JOIN Energizer_DP_Competencia as COMP ON HCOMP.id_competencia= COMP.id_competencia " & _
                    ...
                    "INNER JOIN Regiones as REG ON REG.id_region= TI.id_region  " & _
                    "WHERE HCOMP.tipo_promocionales<>'' " & _
                    "AND COMP.id_competencia <>'1' " & _
                    " " + PeriodoSQL + " " & _
                    " " + RegionSQL + " " & _
                    " " + PromotorSQL + " " & _
                    " " + CadenaSQL + " " & _
                    " " + TiendaSQL + " " & _
                    "ORDER BY PER.nombre_periodo, REG.nombre_region, H.id_usuario, CAD.nombre_cadena, Ti.nombre, HCOMP.tipo_promocionales ", _
                    Me.gridReporte)
    End Sub
