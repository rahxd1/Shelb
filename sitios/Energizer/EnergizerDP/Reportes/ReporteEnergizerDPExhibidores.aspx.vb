...
Partial Public Class ReporteEnergizerDPExhibidores
    ...
    Sub CargarReporte()
        ...
        Dim SQL As String = "SELECT COMP.nombre_competencia, avg(HExh.exhibidores) as Totales " & _
                                "FROM Energizer_DP_Productos_Historial as H " & _
                                "INNER JOIN Energizer_DP_Productos_Historial_Comp as HExh ON H.folio_historial= HExh.folio_historial " & _
                                "INNER JOIN Energizer_DP_Tiendas as TI ON TI.id_tienda= H.id_tienda  " & _
                                "INNER JOIN Regiones as REG ON REG.id_region= TI.id_region  " & _
                                "INNER JOIN Energizer_DP_Competencia as COMP ON COMP.id_competencia = HExh.id_competencia " & _
                                "INNER JOIN Cadenas_Tiendas as CAD ON TI.id_cadena= CAD.id_cadena " & _
                                "INNER JOIN Energizer_DP_Periodos as PER ON H.id_periodo= PER.id_periodo " & _
                                "WHERE COMP.id_competencia <> '1' " & _
                                "AND HExh.exhibidores <>'0' " & _
                                " " + PeriodoSQL + RegionSQL + " " & _
                                " " + PromotorSQL + CadenaSQL + TiendaSQL + " " & _
                                " GROUP BY COMP.nombre_competencia"
        ...
        Using cnn As New SqlConnection(ConexionEnergizer.localSqlServer)
            ...
            For i = 0 To tabla.Rows.Count - 1
                strXML.Append("<set label='" & tabla.Rows(i)("nombre_competencia") & "' value='" & tabla.Rows(i)("totales") & "' />")
            Next i
    ...
    Protected Sub lnkExportar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkExportar.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=Reporte Exhibidore Competencia " + cmbPeriodo.SelectedItem.ToString() + ".xls")
