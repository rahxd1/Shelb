...
Partial Public Class ReporteFrentesCatFerrero
    ...
    Sub CargaGraficas()
        CargaGrafica(Panel1, "1")
        CargaGrafica(Panel2, "2")
        CargaGrafica(Panel3, "3")
        CargaGrafica(Panel4, "4")
    End Sub

    Public Function CargaGrafica(ByVal panel As Panel, ByVal Categoria As String) As Integer
        If cmbPeriodo.SelectedValue <> "" Then
            ...
            Using cnn As New SqlConnection(ConexionFerrero.localSqlServer)
                Dim sqlGrafica As String = "select MAR.id_marca, MAR.nombre_marca, CAT.nombre_categoria, sum(HDET.frentes) as Frentes " & _
                                    ...
                                    "AND HDET.frentes <>0 " & _
                                    " " + RegionSQL + " " & _
                                    " " + PromotorSQL + " " & _
                                    " " + CadenaSQL + " " & _
                                    "GROUP BY MAR.id_marca, MAR.nombre_marca, CAT.nombre_categoria " & _
                                    "ORDER BY MAR.id_marca "

                Dim cmd As New SqlCommand(sqlGrafica, cnn)
                ...
                If tabla.Rows.Count > 0 Then
                    strXML.Append("<chart caption='PORCENTAJE DE FRENTES POR " & tabla.Rows(0)("nombre_categoria") & "' showPercentValues='1' showPercentageInLabel='1' pieSliceDepth='25' showBorder='1' showLabels='1'>")
                End If
    ...
    Sub CargarReporte()
        If cmbPeriodo.SelectedValue <> "" Then
            ...
            Dim Tabla As DataTable = Tablas.Ver.DT(ConexionFerrero.localSqlServer, _
                                "SELECT DISTINCT '10'+ CAT.id_categoria as id_categoria, CAT.nombre_categoria + ' (OTRAS)' as nombre_categoria " & _
                                "FROM AS_Productos as PROD " & _
                                "INNER JOIN AS_Categoria as CAT ON CAT.id_categoria = PROD.tipo_producto " & _
                                "INNER JOIN Marcas as MAR ON MAR.id_marca = PROD.id_marca " & _
                                "UNION ALL " & _
                                "SELECT DISTINCT '100'+ CAT.id_categoria as id_categoria, CAT.nombre_categoria + ' ('+MAR.nombre_marca + ')' as nombre_categoria " & _
                                "FROM AS_Productos as PROD " & _
                                "INNER JOIN AS_Categoria as CAT ON CAT.id_categoria = PROD.tipo_producto " & _
                                "INNER JOIN Marcas as MAR ON MAR.id_marca = PROD.id_marca " & _
                                "WHERE MAR.id_marca = 1 UNION ALL " & _
                                "SELECT DISTINCT CAT.id_categoria, CAT.nombre_categoria +' (totales)' " & _
                                "FROM AS_Productos as PROD " & _
                                "INNER JOIN AS_Categoria as CAT ON CAT.id_categoria = PROD.tipo_producto " & _
                                "INNER JOIN Marcas as MAR ON MAR.id_marca = PROD.id_marca " & _
            ...
            If Tabla.Rows.Count > 0 Then
                For i = 0 To Tabla.Rows.Count - 1
                    IDProducto = Tabla.Rows(i)("id_categoria")
                    NombreProducto = Tabla.Rows(i)("nombre_categoria")
            ...
            CargaGrilla(ConexionFerrero.localSqlServer, _
                        ...
                        " " + NombreCampos + " " & _
                        "FROM(SELECT REG.nombre_region, H.id_usuario, CAD.nombre_cadena, H.nombre_tienda, " & _
                        "'10'+ CAT.id_categoria as id_categoria, HDET.frentes " & _
                        "From AS_Historial as H " & _
                        ...
                        "INNER JOIN Cadenas_Tiendas as CAD ON CAD.id_cadena = H.id_cadena " & _
                        "WHERE H.id_periodo = " & cmbPeriodo.SelectedValue & " And MAR.id_marca <> 1 " & _
                        " " + RegionSQL + " " & _
                        " " + PromotorSQL + " " & _
                        " " + CadenaSQL + " " & _
                        "UNION ALL " & _
                        "SELECT REG.nombre_region, H.id_usuario, CAD.nombre_cadena, H.nombre_tienda, " & _
                        "CAT.id_categoria, HDET.frentes " & _
                        "From AS_Historial as H " & _
                        "FULL JOIN AS_Productos_Historial_Det as HDET ON H.folio_historial= HDET.folio_historial " & _
                        "INNER JOIN AS_Productos as PROD ON PROD.id_producto = HDET.id_producto " & _
                        "INNER JOIN AS_Categoria as CAT ON CAT.id_categoria = PROD.tipo_producto " & _
                        ...
                        "UNION ALL " & _
                        "SELECT REG.nombre_region, H.id_usuario, CAD.nombre_cadena, H.nombre_tienda, " & _
                        "'100'+ CAT.id_categoria as id_categoria, HDET.frentes " & _
                        "From AS_Historial as H " & _
                        "FULL JOIN AS_Productos_Historial_Det as HDET ON H.folio_historial= HDET.folio_historial " & _
                        "INNER JOIN AS_Productos as PROD ON PROD.id_producto = HDET.id_producto " & _
                        "INNER JOIN AS_Categoria as CAT ON CAT.id_categoria = PROD.tipo_producto " & _
                        ...
                        "FOR id_categoria IN (" + Campos + ")) AS PivotTable " & _
    ...
    Protected Sub lnkExportar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkExportar.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=Reporte Frentes por categoria " + cmbPeriodo.SelectedItem.ToString + ".xls")
