...
Partial Public Class ReporteExhibidosFD
    ...
    Sub CargarReporte()
        If cmbPeriodo.SelectedValue <> "" Then
            ...
            CargaGrilla(ConexionFerrero.localSqlServer, _
                        "select REG.nombre_region,H.nombre_tienda, H.id_usuario, COL.nombre_colonia, H.direccion, HDET.[2], HDET.[2A], HDET.[2B], HDET.[2C] " & _
    ...
    Sub CargaGrafica()
        If cmbPeriodo.SelectedValue <> "" Then
            ...
            Dim Tabla As DataTable = Tablas.Ver.DT(ConexionFerrero.localSqlServer, _
                                                   "select HDET.[2], count(HDET.[2]) as cantidad " & _
                                    "FROM Danone_Historial as H " & _
                                    "INNER JOIN Danone_Encuesta as HDET ON H.folio_historial = HDET.folio_historial " & _
                                    "INNER JOIN Danone_CatRutas as RUT ON RUT.id_usuario = H.id_usuario " & _
                                    "INNER JOIN Colonias_Leon as COL ON COL.id_colonia = H.colonia " & _
            ...
            strXML.Append("<chart caption='EL PRODUCTO ESTA EXHIBIDO' showPercentValues='1' showPercentageInLabel='1' pieSliceDepth='25' showBorder='1' showLabels='1'>")

            For i = 0 To Tabla.Rows.Count - 1
                strXML.Append("<set label='" & Tabla.Rows(i)("2") & "' value='" & Tabla.Rows(i)("cantidad") & "'/>")
            Next i
    ...
    Sub CargaGrafica2()
        If cmbPeriodo.SelectedValue <> "" Then
            ...
            Dim Tabla As DataTable = Tablas.Ver.DT(ConexionFerrero.localSqlServer, _
                                                   "select HDET.[2a] as Exhibido, count(HDET.[2a]) as cantidad " & _
                                "FROM Danone_Historial as H " & _
                                "INNER JOIN Danone_Encuesta as HDET ON H.folio_historial = HDET.folio_historial " & _
                                "INNER JOIN Danone_CatRutas as RUT ON RUT.id_usuario = H.id_usuario " & _
                                "INNER JOIN Regiones as REG ON RUT.id_region= REG.id_region AND REG.id_proyecto=30 " & _
                                "WHERE H.id_periodo=" & cmbPeriodo.SelectedValue & " AND HDET.[2A]<>'' " & _
                                " " + RegionSQL + " " & _
                                " " + PromotorSQL + " " & _
                                " " + ColoniaSQL + " " & _
                                "GROUP BY HDET.[2a] " & _
                                "union all " & _
                                "select HDET.[2B] as Exhibido, count(HDET.[2B]) as cantidad " & _
                                "FROM Danone_Historial as H " & _
                                "INNER JOIN Danone_Encuesta as HDET ON H.folio_historial = HDET.folio_historial " & _
                                "INNER JOIN Danone_CatRutas as RUT ON RUT.id_usuario = H.id_usuario " & _
                                "INNER JOIN Regiones as REG ON RUT.id_region= REG.id_region AND REG.id_proyecto=30 " & _
                                "WHERE H.id_periodo=" & cmbPeriodo.SelectedValue & " AND HDET.[2B]<>'' " & _
                                " " + RegionSQL + " " & _
                                " " + PromotorSQL + " " & _
                                " " + ColoniaSQL + " " & _
                                "GROUP BY HDET.[2B] " & _
                                "union all " & _
                                "select HDET.[2C] as Exhibido, count(HDET.[2C]) as cantidad " & _
                                "FROM Danone_Historial as H " & _
                                "INNER JOIN Danone_Encuesta as HDET ON H.folio_historial = HDET.folio_historial " & _
                                "INNER JOIN Danone_CatRutas as RUT ON RUT.id_usuario = H.id_usuario " & _
                                "INNER JOIN Regiones as REG ON RUT.id_region= REG.id_region AND REG.id_proyecto=30 " & _
                                "WHERE H.id_periodo=" & cmbPeriodo.SelectedValue & " AND HDET.[2C]<>'' " & _
                                " " + RegionSQL + " " & _
                                " " + PromotorSQL + " " & _
                                " " + ColoniaSQL + " " & _
                                "GROUP BY HDET.[2C]")
            ...
            Dim TablaTiendas As DataTable = Tablas.Ver.DT(ConexionFerrero.localSqlServer, _
                                             ...
                                             "INNER JOIN Danone_Encuesta as HDET ON H.folio_historial = HDET.folio_historial " & _
                                             "WHERE id_periodo=" & cmbPeriodo.SelectedValue & " AND HDET.[2]='SI' ")
            If TablaTiendas.Rows.Count > 0 Then
                Tiendas = TablaTiendas.Rows(0)("Cantidad")
            End If

            Dim strXML As New StringBuilder()
            strXML.Append("<chart caption='Donde estan exhibidos' yAxisMaxValue='" & Tiendas + 5 & "' showborder='0' bgcolor='FFFFFF' bgalpha='100' rotateLabels='1'  placeValuesInside='1' rotateValues='1' >")
            ...
            For i = 0 To Tabla.Rows.Count - 1
                strXML.Append("<set label='" & Tabla.Rows(i)("Exhibido") & "' value='" & Tabla.Rows(i)("cantidad") & "' />")
            Next i
    ...
    Protected Sub lnkExportar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkExportar.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=Reporte Exhibiciones " + cmbPeriodo.SelectedItem.ToString + ".xls")
