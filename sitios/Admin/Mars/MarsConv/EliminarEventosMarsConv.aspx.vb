...
Partial Public Class EliminarEventosMarsConv
    ...
    Sub SQLCombo()
        ...
        SQLPeriodo = "SELECT DISTINCT orden, nombre_periodo " & _
                    "FROM View_Rutas_Eventos_Conv " & _
                    "ORDER BY orden DESC"
        ...
        SQLRegion = "SELECT DISTINCT id_region, nombre_region " & _
                    "FROM View_Rutas_Eventos_Conv " & _
                    "WHERE id_region<>0 " & _
                    " " + PeriodoSel + QuincenaSel + " " & _
                    "ORDER BY nombre_region"

        SQLUsuario = "SELECT DISTINCT id_usuario FROM View_Rutas_Eventos_Conv " & _
                    "WHERE id_usuario<>'' " & _
                    " " + PeriodoSel + QuincenaSel + RegionSel + " " & _
                    "ORDER BY id_usuario"
    End Sub
            ...
            If Not cmbPeriodo.SelectedValue = "" Then
                If Not cmbQuincena.SelectedValue = "" Then
                    CargaGrilla(ConexionMars.localSqlServer, _
                                "SELECT DISTINCT id_usuario,nombre,id_tienda,nombre_cadena,id_cadena, " & _
                                "CASE estatus_precio when 1 then 'CAPTURADO' when 0 then 'sin captura' end as estatus_precio, " & _
                                "CASE estatus_exhibiciones when 1 then 'CAPTURADO' when 0 then 'sin captura' end as estatus_exhibiciones " & _
                                "FROM View_Rutas_Eventos_Conv  " & _
                                ...
                                "ORDER BY nombre_cadena,nombre", Me.gridRuta)
    ...
    Protected Sub btnEliminarTodos_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEliminarTodos.Click
        BD.Execute(ConexionMars.localSqlServer, _
                    "DELETE FROM Conv_Rutas_Eventos_Exhibiciones " & _
                    "WHERE id_usuario='" & cmbPromotor.SelectedValue & "' " & _
                    "AND orden=" & cmbPeriodo.SelectedValue & "" & _
                    "AND id_quincena='" & cmbQuincena.SelectedValue & "'" & _
                    "AND estatus_exhibiciones=0 " & _
                    "DELETE FROM Conv_Rutas_Eventos_Precios " & _
                    "WHERE id_usuario='" & cmbPromotor.SelectedValue & "' " & _
                    "AND orden=" & cmbPeriodo.SelectedValue & "" & _
                    "AND id_quincena='" & cmbQuincena.SelectedValue & "'" & _
                    "AND estatus_precio=0 ")
        ...
        For i = 0 To CInt(Me.gridRuta.Rows.Count) - 1
            ...
            If chkEliminar.Checked = True Then
                BD.Execute(ConexionMars.localSqlServer, _
                           ...
                           "WHERE id_usuario='" & cmbPromotor.SelectedValue & "' " & _
                           "AND orden=" & cmbPeriodo.SelectedValue & " " & _
                           "AND id_tienda=" & IDTienda & " AND estatus_exhibiciones=0")
            End If
        Next
