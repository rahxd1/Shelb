...
Partial Public Class ReporteAsistenciasSYMDemos
    ...
    Sub CargarReporte()
        If cmbPeriodo.SelectedValue <> "" Then
            ...
            CargaGrilla(ConexionSYM.localSqlServer, _
                        "select REG.nombre_region,US.plaza,CAD.nombre_cadena, H.nombre_tienda, H.apellido_paterno+' '+H.apellido_paterno+', '+H.nombre_demo Demo, " & _
                        "CASE WHEN ISNULL(Dias.[1],0)<>0 then 'Si' else '-' end Viernes, " & _
                        "CASE WHEN ISNULL(Dias.[2],0)<>0 then 'Si' else '-' end Sabado, " & _
                        "CASE WHEN ISNULL(Dias.[3],0)<>0 then 'Si' else '-' end Domingo " & _
                        "FROM Demos_Historial as H " & _
                        "INNER JOIN Cadenas_Tiendas as CAD ON H.id_cadena=CAD.id_cadena " & _
                        "INNER JOIN Usuarios as US ON US.id_usuario=H.id_usuario " & _
                        "INNER JOIN Regiones as REG ON REG.id_region=US.id_region " & _
                        "INNER JOIN (select distinct folio_historial, id_dia " & _
                        "FROM Demos_Productos_Historial_Dias_Det)PVT " & _
                        "PIVOT(avg(id_dia) FOR id_dia  " & _
                        "IN([1],[2],[3])) AS Dias " & _
                        "ON Dias.folio_historial=H.folio_historial " & _
    ...
    Protected Sub lnkExportar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkExportar.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=Reporte asistencia " + cmbPeriodo.SelectedItem.ToString + ".xls")
