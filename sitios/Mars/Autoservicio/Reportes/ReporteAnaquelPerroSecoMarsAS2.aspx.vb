...
Partial Public Class ReporteAnaquelPerroSecoMarsAS2
    ...
    Sub CargarReporte()
        ...
        If cmbPeriodo.SelectedValue <> "" Then
            ...
            CargaGrilla(ConexionMars.localSqlServer, _
                        "SELECT H.nombre_region,H.Ejecutivo, H.area_nielsen,H.codigo,  " & _
                        "H.nombre_cadena,H.nombre_grupo,H.nombre_formato,H.nombre,H.clasificacion_tienda, H.fecha_inicio_quincena, " & _
                        "HDET.[15],HDET.[1],HDET.[5],HDET.[16],(HDET.[15]+HDET.[1]+(HDET.[5]+HDET.[16]))Total,H.O_ps, " & _
                        "cast((Case when HDET.[15]<>0 then((HDET.[15]*100)/cast((HDET.[15]+HDET.[1]+HDET.[5]+HDET.[16])as decimal(9,2)))else 0 end) as decimal(9,2)) Por15, " & _
                        "cast((Case when HDET.[1]<>0 then((HDET.[1]*100)/cast((HDET.[15]+HDET.[1]+HDET.[5]+HDET.[16])as decimal(9,2)))else 0 end) as decimal(9,2)) Por1, " & _
                        "cast((Case when HDET.[5]<>0 then((HDET.[5]*100)/cast((HDET.[15]+HDET.[1]+HDET.[5]+HDET.[16])as decimal(9,2)))else 0 end) as decimal(9,2)) Por5, " & _
                        "(cast((Case when HDET.[15]<>0 then((HDET.[15]*100)/cast((HDET.[15]+HDET.[1]+HDET.[5]+HDET.[16])as decimal(9,2)))else 0 end) as decimal(9,2))+ " & _
                        "cast((Case when HDET.[1]<>0 then((HDET.[1]*100)/cast((HDET.[15]+HDET.[1]+HDET.[5]+HDET.[16])as decimal(9,2)))else 0 end) as decimal(9,2))) PorMars " & _
    ...
    Private Sub gridReporte_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gridReporte.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            If e.Row.Cells(16).Text >= e.Row.Cells(17).Text Then
                e.Row.Cells(16).BackColor = Drawing.Color.GreenYellow
                e.Row.Cells(17).BackColor = Drawing.Color.GreenYellow : Else
