...
Partial Public Class ReporteDetalleExhibicionesEjecutivoMarsAS
    ...
    Sub CargarReporte()
        Dim QuincenaSQL, QuincenaHSQL, RegionSQL, EjecutivoSQL, SupervisorSQL, CadenaSQL As String
        ...
        If cmbPeriodo.SelectedValue <> "" Then
            ...
            CargaGrilla(ConexionMars.localSqlServer, _
                        "select DISTINCT RE.region_mars + '- '+ RE.ejecutivo as EjecutivoMars,RE.nombre_region," & _
                        "RE.clasificacion_tienda,RE.codigo,RE.nombre, RE.nombre_cadena, RE.id_tienda," & _
                        "ISNULL(PVI.Cumplimiento,'NO')PVI,RE.[1],RE.[2],RE.[3],RE.[4],RE.[5]," & _
                        ...
                        "where RE.id_periodo ='" & cmbPeriodo.SelectedValue & "' " + QuincenaHSQL + RegionSQL + EjecutivoSQL + SupervisorSQL + CadenaSQL + " AND CTI.orden<>0)RE " & _
                        "INNER JOIN (SELECT RE.id_tienda, RE.id_usuario, " & _
                        ...
                        "WHERE id_periodo='" & cmbPeriodo.SelectedValue & "' " + QuincenaSQL + " )RE " & _
                        "FULL JOIN(select H.id_tienda,H.id_usuario,H.folio_historial, " & _
                        ...
                        "INNER JOIN AS_Exhibidores_Historial_Det as HDET ON H.folio_historial= HDET.folio_historial   " & _
                        "WHERE H.id_periodo='" & cmbPeriodo.SelectedValue & "'  AND H.id_quincena='" & Q1 & "')H1 ON RE.id_tienda = H1.id_tienda AND RE.id_usuario = H1.id_usuario  " & _
                        "FULL JOIN (select H.id_tienda,H.id_usuario,H.folio_historial, " & _
                        "(HDET.[1]+HDET.[2]+HDET.[3]+HDET.[4]+HDET.[5]+HDET.[6]+HDET.[7]+ " & _
                        "HDET.[8]+HDET.[9]+HDET.[10]+HDET.[11]+HDET.[12]+HDET.[13]+HDET.[14])Total " & _
                        "FROM AS_Historial as H " & _
                        ...
                        "ON RE.id_tienda = H2.id_tienda AND RE.id_usuario = H2.id_usuario)Folio " & _
                        ...
                        "INNER JOIN AS_Clasificacion_Tiendas AS CTI ON CTI.id_clasificacion = TI.id_clasificacion " & _
                        "INNER JOIN AS_Exhibidores_Historial_Det as HDET ON H.folio_historial= HDET.folio_historial " & _
                        "WHERE H.id_periodo ='" & cmbPeriodo.SelectedValue & "' " + QuincenaSQL + RegionSQL + EjecutivoSQL + SupervisorSQL + CadenaSQL + "  AND CTI.orden<>0 )HDET " & _
                        "ON HDET.id_tienda = RE.id_tienda AND HDET.id_usuario = RE.id_usuario " & _
                        "INNER JOIN Regiones as REG ON REG.id_region = TI.id_region " & _
                        "WHERE RE.id_periodo ='" & cmbPeriodo.SelectedValue & "' " + QuincenaSQL + RegionSQL + EjecutivoSQL + SupervisorSQL + CadenaSQL + " and Cumplimiento='SI')PVI " & _
    ...
    Protected Sub lnkExportar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkExportar.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=Reporte detalle exhibiciones por ejecutivo " + cmbPeriodo.SelectedItem.ToString() + ".xls")
    ...
    Private Sub gridReporte_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gridReporte.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            ''//Formato Porcentaje
            If e.Row.Cells(17).Text <> "&nbsp;" Then
                Total(17) = e.Row.Cells(17).Text
                e.Row.Cells(17).Text = Total(17) & "%" : End If

            For i = 23 To 41 Step 3
                If e.Row.Cells(i).Text <> "&nbsp;" Then
                    Total(i) = e.Row.Cells(i).Text
                    e.Row.Cells(i).Text = Total(i) & "%" : End If : Next i

            If e.Row.Cells(23).Text <> "&nbsp;" Then
                Filas1 = Filas1 + 1 : End If

            If e.Row.Cells(29).Text <> "&nbsp;" Then
        ...
        If e.Row.RowType = DataControlRowType.Footer Then
            ...
            ''Formato porcentaje
            e.Row.Cells(17).Text = FormatNumber(Suma(17) / Filas, 2) & " %"
            e.Row.Cells(26).Text = FormatNumber(Suma(26) / Filas, 2) & " %"
            e.Row.Cells(32).Text = FormatNumber(Suma(32) / Filas, 2) & " %"
            e.Row.Cells(35).Text = FormatNumber(Suma(35) / Filas, 2) & " %"
            e.Row.Cells(38).Text = FormatNumber(Suma(38) / Filas, 2) & " %"
            e.Row.Cells(41).Text = FormatNumber(Suma(41) / Filas, 2) & " %"
            e.Row.Cells(23).Text = FormatNumber(Suma(23) / Filas1, 2) & " %"
            e.Row.Cells(29).Text = FormatNumber(Suma(29) / Filas2, 2) & " %"
        End If
    End Sub
End Class